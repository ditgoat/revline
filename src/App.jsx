...
import { Home, Car, ShoppingBag, MapPin, User, Heart, MessageCircle, Share2, X, ChevronRight, Search, Plus, Sparkles, Camera, Image as ImageIcon, LogOut } from "lucide-react";
...
// ---- Mock data (Garage / Shop / Meets are still local placeholders — phase 2) ---
...
function LoadingScreen() {
  return (
    <div className="min-h-[100dvh] w-full flex items-center justify-center bg-[#0c0d0e]">
      <span className="font-mono text-[11px] text-[#5a5d63] tracking-widest">LOADING…</span>
    </div>
  );
}
          ...
          <>
            <div className="flex items-center justify-between mb-3">
              <span className="font-mono text-[11px] tracking-widest text-[#F2C245]">
                {isCollectible ? "COLLECTIBLE" : `PART ${String(item.label ?? item.id).padStart(2, "0")}`}
              </span>
            ...
            <div className="flex items-center justify-between mt-5">
              <span className="font-mono text-2xl text-white">${Number(item.price).toLocaleString()}</span>
...
// ---- New Post flow (writes to Supabase) ------------------------------------

function NewPostSheet({ open, onClose, userId, onPosted }) {
  ...
  const canPost = photo && caption.trim().length > 0 && car.trim().length > 0 && !uploading;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: post, error: postError } = await supabase
        .from("posts")
        .insert({ user_id: userId, car, category, caption, image_url: urlData.publicUrl })
        .select()
        .single();
  ...
  return (
    <div className={`absolute inset-0 z-40 flex items-end transition-opacity duration-300 ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
      ...
      >
        ...
        {error && <p className="font-mono text-[10px] text-[#E4572E] mt-3">{error}</p>}
...
function FeedScreen({ posts, loading, openItem, userId, likedPostIds, onToggleLike, onRefetch }) {
  const [cat, setCat] = useState("ALL");
  const [composerOpen, setComposerOpen] = useState(false);
  const filtered = cat === "ALL" ? posts : posts.filter((p) => p.category === cat);
  const sameSpec = posts.filter((p) => p.car.toUpperCase().includes("CHAMPIONSHIP WHITE"));
  ...
  return (
    <div className="relative h-full">
      <div className="overflow-y-auto no-scrollbar h-full pb-4 screen-enter">
        ...
        {sameSpec.length > 0 && (
          <div className="px-4 pb-3">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="font-mono text-[10px] tracking-widest text-[#8a8d93]">MATCHES YOUR SPEC · CHAMPIONSHIP WHITE</span>
            </div>
        ...
        {loading && <p className="px-4 py-6 font-mono text-[11px] text-[#5a5d63] text-center">LOADING FEED…</p>}
        {!loading && filtered.length === 0 && (
          <p className="px-4 py-10 font-mono text-[11px] text-[#5a5d63] text-center">No posts yet — be the first to post your build.</p>
        )}
            ...
            <div key={post.id} className="screen-enter border-b border-t border-[#2A2C30] pb-4 mb-1" style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <div className="px-4 pt-3">
                ...
                <div className="flex items-center gap-4 mt-3 text-[#8a8d93]">
                  <LikeButton liked={liked} count={post.likes} onToggle={() => onToggleLike(post.id, liked)} />
      ...
      <NewPostSheet open={composerOpen} onClose={() => setComposerOpen(false)} userId={userId} onPosted={onRefetch} />
    </div>
  );
}
  ...
  return (
    <div className="overflow-y-auto no-scrollbar h-full pb-4 screen-enter">
      ...
      <div className="px-4 mt-6">
        ...
        <p className="font-mono text-[9px] text-[#5a5d63] mt-4">Garage editing is coming in the next phase — for now this showcases the layout.</p>
      </div>
    </div>
  );
}
...
function ShopScreen({ openItem }) {
  ...
  return (
    <div className="overflow-y-auto no-scrollbar h-full pb-4 screen-enter">
      ...
      <p className="font-mono text-[9px] text-[#5a5d63] mt-4 px-4">Real checkout/payments are coming in the next phase.</p>
    </div>
  );
}
  ...
  return (
    <div className="overflow-y-auto no-scrollbar h-full pb-4 screen-enter">
      ...
      <div className="px-4 py-6 flex flex-col items-center text-center">
        ...
        <h2 className="font-display text-xl text-white tracking-wide">{username}</h2>
        <p className="font-mono text-[10px] text-[#8a8d93] mt-1 tracking-widest">{email}</p>
        <div className="flex gap-6 mt-4 mb-6">
          {[["0", "CARS"], ["0", "FOLLOWERS"], ["0", "SOLD"]].map(([num, label]) => (
            <div key={label}>
        ...
        <button
          onClick={onSignOut}
          className="flex items-center gap-2 font-mono text-[10px] tracking-widest text-[#E4572E] border border-[#E4572E] px-4 py-2 rounded-lg active:scale-95 transition-transform"
        >
...
export default function RevlineApp() {
  ...
  async function toggleLike(postId, currentlyLiked) {
    ...
    setPosts((prev) => prev.map((p) => (p.id === postId ? { ...p, likes: p.likes + (currentlyLiked ? -1 : 1) } : p)));

    if (currentlyLiked) {
      await supabase.from("post_likes").delete().eq("post_id", postId).eq("user_id", userId);
      await supabase.rpc("increment_likes", { post_id: postId, delta: -1 });
    } else {
  ...
  const screens = {
    ...
    meets: <MeetsScreen />,
    profile: <ProfileScreen email={session.user.email} onSignOut={() => supabase.auth.signOut()} />,
  };
